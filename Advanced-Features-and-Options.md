...
Open GApps by default updates certain libraries that are depended on by certain GApps (especially Facelock) that are stored in `/system/vendor` (or a `/vendor` partition via a symlink). To prevent GApps from touching (removing, adding or replacing) these vendor files use the `skipvendorlibs` keyword.
