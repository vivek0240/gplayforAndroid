...
####Skip Smart Pre-ODEXing of APKs that have no signed classes.dex
On Marshmallow some ROMs are set to be extremely strict when verifying the APK. This can lead to de-odexed APKs to fail verification; because the included classes.dex is not present in the APK's MANIFEST. To avoid this potential issue, Open GApps by default pre-ODEXes these APKs. As positive side-effect this setting also improves the performance of these applications, that is why it is enabled on all 5.0+ builds (if the ROM is using the ART framework). A downside is that it occupies more disk-space on /system, because a copy of the classes.dex has to be kept for regenerating the ODEX when updating the ROM. To disable this feature use the keyword `NoPreODEX`.
